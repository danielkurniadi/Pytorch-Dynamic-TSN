...
class PathLabelPairDataset(BaseDataset, ABC):
    """This class is an abstract base class (ABC) for datasets."""
    ...
    @staticmethod
    def check_path_label_pair_file(filepath):
        if not os.path.isfile(filepath):
            raise FileNotFoundError("File containing (data) path-label-pair doesn't exists. "
                "Filepath: %s" % filepath)
...
def get_params(opts, size):
    ...
    if opts.preprocess == 'resize_and_crop':
        new_h = new_w = opts.input_size
    elif opts.preprocess == 'scale_width_and_crop':
        new_w = opts.input_size
        new_h = opts.input_size * h // w
...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if 'resize' in opt.preprocess:
        osize = [opt.input_size, opt.input_size]
        transform_list.append(transforms.Resize(osize, method))
    elif 'scale_width' in opt.preprocess:
        transform_list.append(transforms.Lambda(lambda img: __scale_width(img, opt.input_size, method)))
